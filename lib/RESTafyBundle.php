...
class CurlHttpClient {
  ...
  function __construct() {
    ...
    if (!\is_resource($ch)) {
      throw new HttpClientException('Unable to create cURL handle.', self::E_INIT);
    }
  ...
  function setOpt($_name_, $_value_) {
    if (\FALSE === \curl_setopt($this->ch, $_name_, $_value_)) {
      throw new Exception("Unable to set option $_name_", self::E_SET_OPT);
    }
  }
  ...
  function execute(HttpRequest $_req_) {
    ...
    case 'TRACE':
      ...
      break;
    default:
      throw new Narvalo\NotImplementedException('Unknown HTTP verb', self::E_UNKNOWN_VERB);
    }
    ...
    if (\FALSE === $body || $errno) {
      ...
      throw new Exception($msg, self::E_EXEC);
    } else {
...
/// Complete HTTP response preamble parser
function http_client_scan($_ch_, $_line_) {
  // NB: each preamble line ends with a CRLF
  // the first empty line (after removing the CRLF) marks the preamble's end
